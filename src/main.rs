...
use std::{collections::HashMap, env, net::SocketAddr, path::PathBuf, sync::Arc, time::{Duration, Instant}};
...
async fn main() {
    ...
    let db = SqlitePoolOptions::new().max_connections(8).connect(&db_url).await.expect("open database");
    ...
    let state = AppState { db, limiter: Default::default(), build_sha };
    let app = app(state);
    let listener = TcpListener::bind(("0.0.0.0", port)).await.expect("bind port");
    axum::serve(listener, app.into_make_service_with_connect_info::<SocketAddr>())
        .with_graceful_shutdown(shutdown_signal()).await.expect("serve application");
}
...
pub fn app(state: AppState) -> Router {
    let api = Router::new()
        ...
        .route("/records/{id}/evidence", post(routes::add_evidence).delete(routes::remove_evidence))
        .route("/records/{id}", axum::routing::delete(routes::delete_record))
        .route("/export", get(routes::export_pack))
        .route("/workspace", axum::routing::delete(routes::delete_workspace).post(routes::create_workspace).get(routes::get_workspace))
...
async fn rate_limit(State(state): State<AppState>, request: Request, next: Next) -> Response {
    let key = request.headers().get("x-forwarded-for").and_then(|v| v.to_str().ok())
        .and_then(|v| v.split(',').next()).unwrap_or("local").trim().to_string();
    let now = Instant::now();
    ...
    if now.duration_since(item.0) >= Duration::from_secs(1) { *item = (now, 0); }
    item.1 += 1;
    if item.1 > 40 {
        return (StatusCode::TOO_MANY_REQUESTS, [(header::RETRY_AFTER, "1")], "Too many requests. Try again in one second.").into_response();
    }
    drop(map);
...
async fn shutdown_signal() {
    ...
    #[cfg(unix)]
    let terminate = async { signal::unix::signal(signal::unix::SignalKind::terminate()).expect("install signal handler").recv().await; };
    #[cfg(not(unix))]
...
mod tests {
    ...
    async fn test_app() -> Router {
        ...
        app(AppState { db, limiter: Default::default(), build_sha: "test-sha".into() })
    }

    #[tokio::test]
    async fn health_reports_build_sha() {
        let response = test_app().await.oneshot(Request::builder().uri("/health").body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn api_rate_limit_returns_retry_after() {
        ...
        for _ in 0..45 {
            let response = service.clone().oneshot(Request::builder().method("POST").uri("/api/demo").header("x-forwarded-for", "203.0.113.8").body(Body::empty()).unwrap()).await.unwrap();
